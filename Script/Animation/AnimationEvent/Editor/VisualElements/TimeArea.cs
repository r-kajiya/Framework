    ...
    class TimeArea : ZoomableArea
    {
        ...
        public float TimeField(Rect rect, int id, float time, float frameRate, TimeFormat timeFormat)
        {
            if (timeFormat == TimeFormat.None)
            {
                ...
                return SnapTimeToWholeFPS(1, frameRate);
            }
        ...
        public string FormatTime(float time, float frameRate, TimeFormat timeFormat)
        {
            if (timeFormat == TimeFormat.None)
            {
                int hDecimals;
                if (frameRate != 0)
                    hDecimals = Mathf.Clamp(-Mathf.FloorToInt(Mathf.Log10(Mathf.Abs(1 / frameRate))), 0, 15);
                else
                    hDecimals = Mathf.Clamp(-Mathf.FloorToInt(Mathf.Log10(Mathf.Abs(shownArea.width / drawRect.width))), 0, 15);
        ...
        {
            int vDecimals = Mathf.Clamp(-Mathf.FloorToInt(Mathf.Log10(Mathf.Abs(shownArea.height / drawRect.height))), 0, 15);
